...
func BuildHTML(current map[string]models.PlayerOwnershipRecord, history map[string][]models.PlayerOwnershipHistoryEntry, summaries []models.ManagerChangeSummary) (string, error) {
	...
	b.WriteString("<html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><title>Fantasy Football Data</title><link rel=\"icon\" href=\"assets/ffd-favicon.svg\" type=\"image/svg+xml\">\n")
	...
	b.WriteString("<style>\n")
	b.WriteString(":root{color-scheme:light;--page-bg:#fff;--text:#222;--heading:#333;--border:#ddd;--subtle-border:#e2e8f0;--surface:#fff;--surface-muted:#f8f9fa;--row:#fafafa;--hover:#f1f5f9;--muted-text:#475569;--chart-bg:#fcfcfd;--grid:#e2e8f0;--grid-light:#f1f5f9;--tick:#94a3b8;--accent:#0057b8}\n")
	b.WriteString(":root[data-theme=\"dark\"]{color-scheme:dark;--page-bg:#171a1f;--text:#edf2f7;--heading:#f8fafc;--border:#4a5568;--subtle-border:#3d4757;--surface:#222831;--surface-muted:#2d3642;--row:#202731;--hover:#354151;--muted-text:#cbd5e1;--chart-bg:#1d242d;--grid:#475569;--grid-light:#334155;--tick:#94a3b8;--accent:#58a6ff}\n")
	...
	b.WriteString(".report-header{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;border-bottom:2px solid var(--accent);padding-bottom:.5rem;margin-bottom:1.5rem}.report-identity{display:flex;align-items:flex-start;gap:.7rem}.report-logo{width:3rem;height:1.8rem;flex:none}\n")
	b.WriteString("h1{margin:0;font-size:2rem;line-height:1.1}.report-title{color:var(--accent)}.report-generated{margin:.25rem 0 0;color:var(--muted-text);font-size:.875rem}.theme-toggle{display:flex;align-items:center;justify-content:space-between;width:100%;border:0;background:transparent;color:var(--text);padding:.45rem .75rem;cursor:pointer;text-align:left}.theme-toggle:hover{background:var(--hover)}\n")
	...
	b.WriteString("@media(max-width:999px){.report-nav{inset:var(--mobile-header-height,4.25rem) 0 auto 0;max-height:calc(100vh - var(--mobile-header-height,4.25rem))}}\n")
	b.WriteString("@media(max-width:999px){.chart-container{padding:.5rem;box-sizing:border-box;overflow:hidden}.trend-chart{min-height:0;height:auto;aspect-ratio:11/8}.legend-container{max-height:180px}}\n")
	b.WriteString("@media(max-width:999px){#team-changes-table thead th:first-child,#team-changes-table tbody tr:not(.manager-detail-row) td:first-child{display:none}}\n")
	b.WriteString("@media(max-width:999px){.mobile-header{border-bottom:2px solid var(--accent)}.mobile-header .report-identity{align-items:center;gap:.55rem}.mobile-header .report-logo{width:2.5rem;height:1.5rem}.mobile-title{font-size:1.65rem}h2{font-size:1.35rem}.menu-button:hover,.menu-button:focus-visible{outline-color:var(--accent)}}\n")
	b.WriteString(".sidebar-logo{display:none}@media(min-width:1000px){.sidebar-logo{display:block;width:9rem;height:5.4rem;margin:0 0 1.5rem .75rem}}\n")
	b.WriteString("</style></head><body>\n")
	b.WriteString(fmt.Sprintf("<header class=\"mobile-header\"><div class=\"report-identity\"><img class=\"report-logo\" src=\"assets/ffd-logo.svg\" alt=\"FFD\"><div><h1 class=\"mobile-title report-title\">Fantasy Football Data</h1><p class=\"mobile-generated\">Updated: %s</p></div></div><button class=\"menu-button\" type=\"button\" aria-label=\"Open navigation menu\" aria-controls=\"report-nav-content\" aria-expanded=\"false\"><span class=\"hamburger\"></span></button></header>\n", generatedAt))
	b.WriteString(fmt.Sprintf("<header class=\"report-header\"><div><h1 class=\"report-title\">Fantasy Football Data</h1><p class=\"report-generated\">Updated: %s</p></div></header>\n", generatedAt))
	b.WriteString("<nav class=\"report-nav\" aria-label=\"Report sections\"><img class=\"sidebar-logo\" src=\"assets/ffd-logo.svg\" alt=\"FFD\"><div id=\"report-nav-content\" class=\"report-nav-content\"><div class=\"nav-group\"><span class=\"nav-label\">Statistics</span><a href=\"#player-ownership\">Player ownership</a><a href=\"#team-changes\">Team changes</a></div><div class=\"nav-group\"><span class=\"nav-label\">Trends</span><a href=\"#historical-trends\">Historical trends</a></div><div class=\"nav-settings\"><span class=\"nav-label\">Settings</span><button class=\"theme-toggle\" type=\"button\" aria-label=\"Toggle dark mode\" title=\"Toggle dark mode\">Dark mode</button></div></div></nav>\n")
	...
	b.WriteString(";(function(){var header=document.querySelector('.mobile-header');if(!header)return;function updateMobileHeaderHeight(){document.documentElement.style.setProperty('--mobile-header-height',header.getBoundingClientRect().height+'px')}window.addEventListener('resize',updateMobileHeaderHeight);updateMobileHeaderHeight()}())\n")
	b.WriteString("</script>\n")
	b.WriteString("<script>(function(){var menuButton=document.querySelector('.menu-button');var navigation=document.querySelector('.report-nav');var header=document.querySelector('.mobile-header');if(menuButton&&navigation){menuButton.addEventListener('click',function(){if(header){document.documentElement.style.setProperty('--mobile-header-height',header.getBoundingClientRect().height+'px')}var open=navigation.classList.toggle('is-open');menuButton.setAttribute('aria-expanded',String(open));menuButton.setAttribute('aria-label',open?'Close navigation menu':'Open navigation menu')})}var links=document.querySelectorAll('.report-nav a[href^=\"#\"]');Array.prototype.forEach.call(links,function(link){link.addEventListener('click',function(){if(navigation){navigation.classList.remove('is-open')}if(menuButton){menuButton.setAttribute('aria-expanded','false');menuButton.setAttribute('aria-label','Open navigation menu')}})});var sections=Array.prototype.map.call(links,function(link){return document.querySelector(link.getAttribute('href'))}).filter(Boolean);function update(){var current=sections[0];sections.forEach(function(section){if(section.getBoundingClientRect().top<=120){current=section}});links.forEach(function(link){link.setAttribute('aria-current',document.querySelector(link.getAttribute('href'))===current?'true':'false')})}window.addEventListener('scroll',update,{passive:true});window.addEventListener('hashchange',update);update()}())</script>\n")
