...
The first release captures Guy Sports team snapshots on a timestamped, additive basis, derives historical player ownership and manager change data from the stored raw snapshots, and renders a static HTML report that can be read locally and hosted on GitHub Pages without live network access. The default implementation language is Go, with YAML as the preferred raw storage format. Any alternative language or storage format requires explicit approval before use.
