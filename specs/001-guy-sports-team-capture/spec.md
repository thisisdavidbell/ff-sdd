...
- **FR-008**: The system MUST track changes between successive team snapshots so that manager-level change counts can be calculated accurately regardless of the time gap between captures. A substitution/transfer (1 player removed, 1 player added) MUST be counted as 1 change (`change_count = max(len(added), len(removed))`), and total manager changes MUST equal the sum of event change counts.
- **FR-009**: The system MUST compute player ownership at any capture timestamp $T$ by evaluating the active team composition for ALL known managers in the league as of timestamp $T$ (taking each manager's latest snapshot captured on or before $T$). Partial capture runs where only a subset of managers created new snapshots MUST NOT cause player ownership counts to drop for players on unchanged teams.
- **FR-010**: The system MUST capture selection counts as historical values for each capture event so that changes in player popularity over time can be shown. Historical player ownership counts and current ownership counts MUST remain 100% consistent across `player-ownership.yaml`, the HTML ownership table, and the historical trends line chart.
- **FR-011**: The system MUST compute each manager's total team changes within a season using historical team snapshots.
- **FR-012**: The system MUST show, in human-readable HTML output, the date of each manager's latest team change and provide an interactive drill-down view (e.g. expandable row on click) for individual change events, displaying the timestamp, event change count, and full player names for added (`+`) and removed (`-`) players.
...
- **SC-003**: Player selection counts are reported accurately for current state and all historical capture timestamps by evaluating every manager's active snapshot as of that timestamp. Partial capture runs where only a subset of teams change do not distort ownership totals or line graph trends. Current counts in `player-ownership.yaml`, the HTML table, and the line graph match consistently.
- **SC-004**: Each manager's total team-change count is calculated accurately from sequential snapshots without overcounting unchanged states, where swapping 1 player out for 1 player in equals 1 change, and total changes equals the sum of event change counts.
