...
- **FR-008**: The system MUST track changes between successive team snapshots so that manager-level change counts can be calculated accurately regardless of the time gap between captures. A player change is defined as a player substitution/swap (1 player removed, 1 player added). Removing and adding 2 players in a single snapshot delta MUST count as 2 player changes (`change_count = max(len(added), len(removed))`). A manager's total changes across the season MUST equal the cumulative count of all player removal/addition pairs across all change events in their event history.
- **FR-009**: The system MUST compute player ownership at any capture timestamp $T$ by evaluating the active team composition for ALL known managers in the league as of timestamp $T$ (taking each manager's latest snapshot captured on or before $T$). When a capture run occurs and only a subset of managers have changed teams (creating new raw snapshot files while unchanged managers create none), player ownership counts at timestamp $T$ MUST be calculated by combining the updated teams with the unchanged teams' latest active snapshots. This ensures player ownership totals and historical trend points accurately reflect the net ownership changes from the updated teams without improperly omitting unchanged teams from the league total.
